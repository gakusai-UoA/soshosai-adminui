...
function ProjectsPage() {
    ...
    const [newProject, setNewProject] = useState({ projectName: '', destinationUrl: '', adminUser: Cookies.get('staffId') });
    ...
    const userHasAccess = (project) => {
        ...
        return project.adminUserId === null || 
               project.adminUserId === 'all' || 
               project.adminUserId === currentUserId;
    };

    const fetchProjects = async () => {
        try {
            const response = await fetch('https://api.100ticket.soshosai.com/projects');
            if (response.ok) {
    ...
    const handleCreateProject = async (e) => {
        e.preventDefault();
        try {
            const response = await fetch('https://api.100ticket.soshosai.com/projects/create', {
                method: 'POST',
                headers: {
    ...
    const handleDeleteProject = async (projectId) => {
        ...
        try {
            const response = await fetch(`https://api.100ticket.soshosai.com/projects/${projectId}`, {
                method: 'DELETE',
            });
            if (response.ok) {
                fetchProjects();
    ...
    const handleUpdateProject = async (e) => {
        ...
        try {
            const response = await fetch(`https://api.100ticket.soshosai.com/projects/${editingProject.id}`, {
                method: 'PUT',
                headers: {
                    'Content-Type': 'application/json',
                },
                ...
                <form onSubmit={handleCreateProject}>
                    ...
                    <div className="mb-4">
                        ...
                        <input
                            ...
                            onChange={(e) => setNewProject({...newProject, destinationUrl: e.target.value})}
                            className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                            required
                        />
                    </div>
                            ...
                            {projects.filter(userHasAccess).map((project) => (
                                <tr key={project.id} className="border-b">
                                    ...
                                    <td className="px-4 py-2">
                                        <a href={project.destination_url} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">
                                            {project.destination_url}
                                        </a>
                                    </td>
                                    <td className="px-4 py-2">{new Date(project.createdAt).toLocaleString('ja-JP')}</td>
                                    <td className="px-4 py-2">
                                        <button
                        ...
                        <form onSubmit={handleUpdateProject}>
                            ...
                            <div className="mb-4">
                                ...
                                <input
                                    ...
                                    onChange={(e) => setEditingProject({...editingProject, destination_url: e.target.value})}
                                    className="shadow appearance-none border rounded w-full py-2 px-3 text-gray-700 leading-tight focus:outline-none focus:shadow-outline"
                                    required
                                />
                            </div>
